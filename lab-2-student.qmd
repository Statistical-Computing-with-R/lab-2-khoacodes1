...
```{r}
#| label: load-data
df <- read_csv(here("data/surveys.csv"), show_col_types = FALSE)
...
The dimensions of the data frame contains 30,463 rows and 15 columns. 

**3. What are the data types of the variables in this dataset?**

<!-- You can either write code here or in the code chunk for #1, but I **must** see the code you used to address this question!  -->

The data types in the dataset contains double, character, and date format.
...
The relationship between rodent weight and hindfoot length have a very weak correlation. As most patterns from many different species showcase a vertical or horizontal or clumped up data distribution showing that the relationship is weak, if it was strong the pattern should be a linear growing line (x). The rodent weight does differ by species most species have a weight distribution between 0-100 grams, whereas albigula have a substantial distribution over 200 grams, even spectabilis rodent occupying the 100-200 grams.  
...
Since, our p-value at 2e-16 is smaller than our significance level at 0.05, we reject the null hypothesis that the population mean weight is the same for all fourteen rodent species, and we have strong evidence that at least one rodent species have a different population mean weight. 
